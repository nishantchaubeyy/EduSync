...
export default function StudentDashboardPage() {
    ...
    // While Clerk is loading basic auth state, show a placeholder skeleton
    if (!isLoaded) {
        return (
            <div className="dashboard-page animate-pulse p-8">
                <div className="h-64 bg-slate-100 rounded-[3rem] mb-12 shadow-sm" />
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <div className="h-48 bg-slate-100 rounded-3xl" />
                        <div className="h-48 bg-slate-100 rounded-3xl" />
                    </div>
                    <div className="lg:col-span-1 h-[30rem] bg-slate-100 rounded-3xl" />
                </div>
            </div>
        );
    }

    // Map Convex enrollments to UI structure, fallback to mock data if empty OR Convex hasn't returned yet
    const inProgressCourses = (enrollments && enrollments.length > 0)
        ? enrollments.map((e, idx) => ({
            id: e.courseId || `convex-${idx}`,
            title: typeof e.course === 'object' && e.course !== null ? (e.course as any).title : "Enrolled Course",
            category: "Enrolled",
            progress: 0,
